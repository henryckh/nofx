...
type Manager struct {
	...
	onAICall   func(runID string, cycle int, provider string) error
}
...
func (m *Manager) SetOnAICall(callback func(runID string, cycle int, provider string) error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.onAICall = callback
}
...
func (m *Manager) Start(ctx context.Context, cfg BacktestConfig) (*Runner, error) {
	...
	// Set AI call callback if configured
	if m.onAICall != nil {
		runID := cfg.RunID
		runner.onAICall = func(cycle int, provider string) error {
			return m.onAICall(runID, cycle, provider)
		}
	}
