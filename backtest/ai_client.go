...
func configureMCPClient(cfg BacktestConfig, base mcp.AIClient) (mcp.AIClient, error) {
	...
	case "system_default", "system":
		// System default uses a pre-configured DeepSeek API key
		// The API key should be set via environment variable or system config
		ds := mcp.NewDeepSeekClientWithOptions()
		// API key will be set from system config or environment
		if cfg.AICfg.APIKey != "" {
