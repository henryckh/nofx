...
// OnAICallCallback is called when AI is invoked (for credit deduction, etc.)
type OnAICallCallback func(cycle int, provider string) error
...
type Runner struct {
	...
	onAICall OnAICallCallback // Callback for AI invocation (e.g., credit deduction)
}
...
func NewRunner(cfg BacktestConfig, mcpClient mcp.AIClient) (*Runner, error) {
	...
	r := &Runner{
		...
		aiCache:        aiCache,
		cachePath:      cachePath,
		onAICall:       nil, // Set by manager if needed
	}
...
func (r *Runner) stepOnce() error {
	...
	if shouldDecide {
		...
		if !fromCache {
			// Deduct credits if using system_default and callback is set
			if r.cfg.AICfg.Provider == "system_default" && r.onAICall != nil {
				if err := r.onAICall(callCount, r.cfg.AICfg.Provider); err != nil {
					log.Printf("⚠️ Credit deduction failed for cycle %d: %v", callCount, err)
					// Continue with AI call even if credit deduction fails (for demo)
				}
			}
...
func (r *Runner) executeDecision(dec decision.Decision, priceMap map[string]float64, ts int64, cycle int) (logger.DecisionAction, []TradeEvent, string, error) {
	...
	case "update_stop_loss", "update_take_profit":
		// In backtest, stop loss and take profit are not tracked as explicit orders.
		// These actions are informational only - they don't affect the backtest simulation.
		// The backtest account uses liquidation price based on leverage, not explicit stop loss.
		note := fmt.Sprintf("更新%s: %s", 
			map[string]string{
				"update_stop_loss":   "止损",
				"update_take_profit": "止盈",
			}[dec.Action],
			dec.Symbol)
		if dec.NewStopLoss > 0 {
			note += fmt.Sprintf(" (新止损: %.4f)", dec.NewStopLoss)
		}
		if dec.NewTakeProfit > 0 {
			note += fmt.Sprintf(" (新止盈: %.4f)", dec.NewTakeProfit)
		}
		return actionRecord, nil, note, nil

	case "partial_close":
		// Partial close is handled the same as regular close, but with a specific quantity
		side := ""
		for _, pos := range r.account.Positions() {
			if pos.Symbol == strings.ToUpper(dec.Symbol) {
				side = pos.Side
				break
			}
		}
		if side == "" {
			return actionRecord, nil, "", fmt.Errorf("no position found for %s", dec.Symbol)
		}
		
		qty := r.determineCloseQuantity(dec.Symbol, side, dec)
		if qty <= 0 {
			return actionRecord, nil, "", fmt.Errorf("invalid partial close qty")
		}
		posLev := r.account.positionLeverage(dec.Symbol, side)
		realized, fee, execPrice, err := r.account.Close(dec.Symbol, side, qty, fillPrice)
		if err != nil {
			return actionRecord, nil, "", err
		}
		actionRecord.Quantity = qty
		...
		trade := TradeEvent{
			...
			Leverage:      posLev,
			Cycle:         cycle,
			PositionAfter: r.remainingPosition(dec.Symbol, side),
		}
		return actionRecord, []TradeEvent{trade}, "", nil
...
func (r *Runner) determineCloseQuantity(symbol, side string, dec decision.Decision) float64 {
	for _, pos := range r.account.Positions() {
		if pos.Symbol == strings.ToUpper(symbol) && pos.Side == side {
			// If ClosePercentage is specified (for partial_close), use it
			if dec.Action == "partial_close" && dec.ClosePercentage > 0 && dec.ClosePercentage <= 100 {
				return pos.Quantity * (dec.ClosePercentage / 100.0)
			}
			// Otherwise close the full position
			return pos.Quantity
		}
	}
	return 0
